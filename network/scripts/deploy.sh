...
approveForMyOrg() {
  ...
  echo ${PACKAGE_ID}
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma-net.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG}   res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
export PEER0_TRANSPORTER_CA=${PWD}/organizations/peerOrganizations/transporter.pharma-net.com/peers/peer0.transporter.pharma-net.com/tls/ca.crt
  ...
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma-net.com --channelID pharmachannel --name pharma --version 1.0 --sequence 1 --signature-policy "OR('manufaturerMSP.peer')" --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_MANUFACTURER_CA --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_DISTRIBUTOR_CA --peerAddresses localhost:11051 --tlsRootCertFiles $PEER0_RETAILER_CA --peerAddresses localhost:13051 --tlsRootCertFiles $PEER0_CONSUMER_CA --peerAddresses localhost:15051 --tlsRootCertFiles $PEER0_TRANSPORTER_CA
...
## Install chaincode on peer0 and peer1 of manufacturer, distributor, retailer, consumer and tranporter
infoln "Installing chaincode on peer0.manufacturer..."
installChaincode 0 1
infoln "Installing chaincode on peer1.manufaturer..."
installChaincode 1 1
infoln "Install chaincode on peer0.distributor..."
installChaincode 0 2
infoln "Install chaincode on peer1.distributor..."
installChaincode 1 2
infoln "Install chaincode on peer0.retailer..."
installChaincode 0 3
infoln "Install chaincode on peer1.retailer..."
installChaincode 1 3
infoln "Install chaincode on peer0.consumer..."
installChaincode 0 4
infoln "Install chaincode on peer1.consumer..."
installChaincode 1 4
infoln "Install chaincode on peer0.transporter..."
installChaincode 0 5
infoln "Install chaincode on peer1.transporter..."
...
## now approve also for transporter
approveForMyOrg 0 5
...
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.pharma-net.com --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${CC_END_POLICY} ${CC_COLL_CONFIG} --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $PEER0_MANUFACTURER_CA --peerAddresses localhost:9051 --tlsRootCertFiles $PEER0_DISTRIBUTOR_CA --peerAddresses localhost:11051 --tlsRootCertFiles $PEER0_RETAILER_CA --peerAddresses localhost:13051 --tlsRootCertFiles $PEER0_CONSUMER_CA --peerAddresses localhost:15051 --tlsRootCertFiles $PEER0_TRANSPORTER_CA
