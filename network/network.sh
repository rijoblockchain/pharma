...
function createOrgs() {
  ...
  while :
    ...
    infoln "Creating Transporter Identities"
...
function deployCC() {
  scripts/deploy.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
COMPOSE_FILE_BASE=docker/docker-compose-pharma-net.yaml
