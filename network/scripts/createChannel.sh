...
createChannelGenesisBlock() {
	...
	set -x
	configtxgen -profile PharmaApplicationGenesis -outputBlock ./channel-artifacts/${CHANNEL_NAME}.block -channelID $CHANNEL_NAME
	res=$?
	{ set +x; } 2>/dev/null
...
setAnchorPeer() {
  PEER=$1
  ORG=$2
  docker exec cli ./scripts/setAnchorPeer.sh $PEER $ORG $CHANNEL_NAME 
}
